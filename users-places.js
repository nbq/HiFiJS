...
//
//  Modded by Coal / Jason Hawks to expand window width and add place name

var usersWindow = (function () {

    var WINDOW_WIDTH_2D = 200,
        WINDOW_MARGIN_2D = 12,
    ...
    function updateWindow() {
        ...
        for (i = 0; i < usersOnline.length; i += 1) {
            ...
            if (user.username !== myUsername && user.online) {
                ...
                linesOfUsers.push(i);
                displayText += "\n" + user.username;
                if (user.location.root) { displayText += " @ " + user.location.root.name }
                //else if (user.location.domain.network_address) { displayText += " at " + user.location.domain.network_address }
            }
        }
